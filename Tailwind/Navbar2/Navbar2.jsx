...
import { Expand,IconButton } from "..";
const sample = {
    fixed: "fixed top-0 left-0 w-full animate_animated animate_slideInDown",
    // fixedUp : "fixed top-0 left-0 w-full animate_animated animate_fadeInDown",
    ...
    const design = (
        ...
        `}>
            ...
            <Expand state={show} className="fixed bottom-0 left-0 w-full bg-slate-900 z-50">
                ...
                {
                    toolbar ? <div className="sm:hidden flex gap-3 p-4">
                        {
                            toolbar.map((item, index) => {
                                return <div key={index}>{item.label}</div>
                            })
